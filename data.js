        ...
        const carData = [
            ...
            function createCarArticle(car) {
                    ...
                    article.classList.add('flex', 'flex-col', 'w-[229px]', 'h-[265.75px]', 'rounded-[7px]', 'overflow-hidden', 'custom-shadow');

                    const imageDiv = document.createElement('div');
                    imageDiv.classList.add('h-[170.75px]');
                    const img = document.createElement('img');
                    img.classList.add('h-[170.75px]', 'w-[229px]');
                    ...
                    infoDiv.classList.add('px-[10px]', 'pt-[7px]', 'h-[95px]', 'flex', 'flex-col', 'leading-[20px]');

                    const price = document.createElement('h3');
                    price.classList.add('font-bold', 'text-[18px]');
                    price.textContent = `${car.price} ${car.currency}`;
                    infoDiv.appendChild(price);

                    const brandModelDiv = document.createElement('div');
                    brandModelDiv.innerHTML = `<span>${car.brand}</span> <span>${car.model}</span>`;
                    infoDiv.appendChild(brandModelDiv);

                    const yearEngineOdometerDiv = document.createElement('div');
                    yearEngineOdometerDiv.innerHTML = `
                        <span>${car.year}</span>,
                        <span>${car.engine}</span>,
                        <span>${car.odometer} ${car.odometerUnit}</span>
                    `;
                    infoDiv.appendChild(yearEngineOdometerDiv);

                    const cityDateDiv = document.createElement('div');
                    cityDateDiv.classList.add('text-[14px]', 'text-[#8d94ad]');
                    cityDateDiv.innerHTML = `<span>${car.city}</span>, <span>${car.dates}</span>`;
                    infoDiv.appendChild(cityDateDiv);
                ...
                function renderCarArticles() {
                    const container = document.querySelector('section');
                    carData.forEach(car => {
                        const carArticle = createCarArticle(car);
                        container.appendChild(carArticle);
                    });
        }
